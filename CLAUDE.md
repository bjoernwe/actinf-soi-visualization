...
- **Change one thing at a time; respect the module boundaries (types in `stages.ts`/`diagram.ts`, content in `content/`, engine in `components/flow-diagram/engine.ts`/`comments.ts`, shell in `components/`, per-page wiring in `pages/`); keep diffs small.** Commit at each step.
- A new page is a `DiagramConfig` + `Stage[]` in `content/`, a one-line bootstrap in `pages/` (`mountPage(config, stages)`), an `.html` entry, and a line in `vite.config.ts`'s `build.rollupOptions.input`. It should not need changes to `components/flow-diagram/`.
